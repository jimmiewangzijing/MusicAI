...
这是一个基于 Web 的音乐分轨及乐谱生成工具，旨在自动化音乐分析和转录的核心流程。用户可以通过一个简洁的网页界面上传音频文件，系统将自动执行音轨分离、旋律提取、歌词识别，并生成多种格式的乐谱文件。
...
使用我们提供的 `setup.py` 脚本可以自动完成所有依赖安装和模型下载：
...
- 检查 Python 版本
- 安装所有必需的 Python 依赖包
- 下载 Spleeter 和 Vosk 语音识别模型到缓存目录
- 验证安装是否成功
...
4. **模型会自动按需下载**
   现在预训练模型已转为外部依赖，当您首次运行应用时，系统会自动检查并下载缺失的模型到缓存目录。
